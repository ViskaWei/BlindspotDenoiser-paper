...
\subsection{Downstream surface-gravity calibrator transfer}
\label{sec:results_downstream_logg}

The reconstruction metric is not the endpoint for Galactic Archaeology. Surface gravity is the stellar label most directly tied to the dwarf--giant decision, so we next ask whether the denoised spectra remain readable to an estimator trained only on clean spectra. We train a full-spectrum $\log g$ calibrator on clean spectra, freeze it, and apply it unchanged to noisy, smoothed, and blindspot-denoised held-out spectra.
...
The clean-input row reaches $R^{2}=0.940$, so the $\log g$ task itself is not degenerate: the fixed mapping encodes gravity-discriminating spectral information in this BOSZ support. Against that reference, the blindspot output recovers the information that the noisy input loses --- the centered robust scatter falls from $1.262$ on the noisy input to $0.801$~dex, MAE falls from $0.958$ to $0.670$~dex, and the noisy-input bias is largely removed. The two validation-tuned smoothers do not transfer: their residual statistics stay close to the noisy-input scale. The improvement therefore depends on the line-preserving reconstructions characterized in Section~\ref{sec:results_physical}, not on generic noise reduction by a conventional smoother.
