...
\begin{itemize}
    ...
    \item Line-sensitive and high-noise-window diagnostics evaluated alongside aggregate reconstruction error: Ca\,\textsc{ii} triplet equivalent-width (EW) error and an \mbox{O$_2$} A-band stress test in a high-noise atmospheric window.
\end{itemize}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\linewidth]{figs/fig_architecture_blindspot.pdf}
\caption{Simplified blindspot architecture. The red center value $y_{\lambda}$ is held out; the right context is implemented as reverse $\to$ shifted-causal $U_{\theta}$ $\to$ flip back, and $F_{\theta'}$ predicts $\hat{x}_{\lambda}$ from $u^L_{\lambda}$ and $u^R_{\lambda}$.}
...
% === polarity-flip diagnostic commented out per 2026-06-30 decision; kept for recovery ===
% Continuum-relative polarity uses a robust continuum-level reference for each normalized clean spectrum. For spectrum $s$, let $m_s$ be the per-spectrum median flux of the clean normalized spectrum (a fixed reference level for the diagnostic, not a fitted physical continuum). The polarity-flip statistic measures whether an informative pixel in $y$ or $\hat{x}$ lies on the same side of $m_s$ as the clean flux, counting sign errors in local contrast (line filling / contrast inversion) at the pixel level.
% [Equation eq:polarity_flip omitted here to avoid a heavy-LaTeX edit; the full original block is preserved in git HEAD:main.tex. It defined the informative set P = {(s,lambda): |x_s-m_s| >= 0.02 m_s}, the subsets P- (x<m) and P+ (x>m), and p_flip(X;Q) = fraction of pixels in Q where sign(X_s-m_s) != sign(x_s-m_s).]
%
...
% polarity metric commented out (2026-06-30): originally "\item \textbf{Polarity-flip rate}: Equation eq:polarity_flip, reported on P, P-, and P+."
...
\hline
% polarity rows commented out (2026-06-30):
...
\begin{enumerate}
    \item \textbf{Reconstruction gain.} On the $10{,}000$-spectrum reconstruction test split, mean per-spectrum reconstruction S/N increases from $7.6$ for the noisy input to $122$ for the blindspot output. The central-visible control remains at $7.6$, supporting the interpretation that target-bin exclusion prevents a copy shortcut rather than merely adding network capacity.
...
This appendix checks that the reconstruction-S/N gain of Section~\ref{sec:results_denoising} and the line-preservation gain of Section~\ref{sec:results_physical} are distributed across the test-set input-S/N range and across diagnostic lines, rather than driven by a small subset of spectra.

\paragraph{Full line-sensitive diagnostics.}
Table~\ref{tab:main_results_full} is the full-precision version of the compact main-text line-preservation table. It preserves the exact bootstrap details while keeping the Results section within the main-text digit budget.
