...
The scalar $\bar{\sigma}^2$ represents the overall noise level of the spectrum and $\bar{\sigma}$ has the same unit of flux as $y$. We use the sum of the per-bin variances rather than the full vector $\{\sigma_{\lambda}^2\}_{\lambda=1}^{N}$ because the full vector contains wavelength-specific information from the noiseless simulated spectrum that could cause data leakage. Hence the full, pixel-wise variance vector is used only to draw $\epsilon_{\lambda}$ in Equation~\ref{eq:noise_model_scalar} and the network receives only the scalar noise summary $\bar{\sigma}$, as defined in Equation~\ref{eq:blindspot_prediction}. The clean reference $x$ is completely withheld during training and used only for evaluation.
