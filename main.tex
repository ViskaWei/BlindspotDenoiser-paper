...
This paper evaluates a one-dimensional blindspot denoiser on continuum-normalized late-type BOSZ spectra with a PFS-like setup and independent zero-mean per-pixel noise. The denoiser is trained against noisy targets with the target wavelength excluded from the receptive field; clean spectra do not enter the denoiser training loss. Under the selected checkpoint and fixed evaluation protocol, the evidence separates into three claims: reconstruction improves, line diagnostics remain favorable, and a surface-gravity estimator trained on clean spectra transfers better without refitting.
...
\begin{enumerate}
    ...
    \item \textbf{Line preservation.} Median Ca\,\textsc{ii} relative EW errors for the $8498$, $8542$, and $8662$~\AA{} lines fall to $0.121$, $0.098$, and $0.147$, while all non-continuum polarity flips fall from $0.276$ to $0.00302$. These diagnostics argue that the reconstruction retains local absorption-line contrast rather than only improving the global norm.
...
These claims remain limited to the current simulated benchmark and evaluation protocol. They do not establish abundance preservation, thresholded dwarf--giant classification, survey deployment performance, or robustness to correlated sky-subtraction residuals, extraction covariance, line-spread-function drift, continuum-estimation error, or stellar regimes outside the simulated faint late-type support. The next stage is to test the same fixed-estimator transfer question on observed spectra or on simulations with those structured errors, while keeping the downstream estimator unchanged.
