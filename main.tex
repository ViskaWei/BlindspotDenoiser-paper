...
\paragraph{Scope of the simulated benchmark.} The evaluated setting is the independent, zero-mean simulator described in Section~\ref{sec:method}, with continuum-normalized late-type spectra and the PFS-like wavelength, resolution, and magnitude range in Table~\ref{tab:data_summary}. Real survey deployment introduces correlated sky-subtraction residuals, extraction covariance, and a varying instrumental line-spread function that are outside the independent zero-mean simulator tested here. The same frozen-evaluation logic should be reused when moving to survey-specific residuals, extraction artifacts, or other stellar supports: train or select the denoiser without clean targets, freeze it, and then test reconstruction, line preservation, and downstream clean-trained transfer on a disjoint evaluation set.
...
The result is therefore a transfer claim, not a deployment claim. It shows that noisy-only training is sufficient for the line-shape and continuum information that a frozen clean-trained estimator needs on this benchmark, and it leaves open whether the same construction transfers under correlated sky-subtraction residuals, extraction covariance, varying instrumental line-spread function, or stellar regimes outside the late-type, faint-magnitude scope tested here. Whether residual gains in surface-gravity scatter translate into a thresholded dwarf--giant classification metric on labeled samples, and whether the line-preservation diagnostics extend to abundance and radial-velocity measurements, are the natural next tests under the same frozen-evaluation protocol.
