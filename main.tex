...
Table~\ref{tab:appendix_baselines} resolves the validation-selected baselines onto the same test split, augmenting the baseline rows of Table~\ref{tab:main_baselines} with median and bracket statistics and Ca\,\textsc{ii} EW error, so that the line-preservation and reconstruction columns are measured against the same baseline rows.
