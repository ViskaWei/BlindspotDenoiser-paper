...
The clean-reference $R^{2}$ in Table~\ref{tab:downstream_logg} confirms that the fixed map extracts gravity-discriminating information on this BOSZ support. Against that reference, the blindspot row recovers most of the $\sigma_{\rm rob}$, MAE, and bias inflation that the noisy input introduces, while the smoother rows stay near the noisy-input scale. The reduction therefore tracks the line-preserving reconstructions of Section~\ref{sec:results_physical}, not generic noise removal.
